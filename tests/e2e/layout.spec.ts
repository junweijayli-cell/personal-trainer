...
for (const locale of ['en', 'zh'] as const) {
  test(`public and account-entry typography (${locale})`, async ({ page }, testInfo) => {
    ...
    await expect(page.locator('.price-options')).toContainText(locale === 'zh' ? 'US$5 / 月' : 'US$5 / month');
    await expect(page.locator('.price-options')).toContainText(locale === 'zh' ? 'US$30 / 年' : 'US$30 / year');
