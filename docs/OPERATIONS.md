...
Create one TrainWell product and two fixed, licensed recurring Prices: **US$10 every month** (`unit_amount=1000`, `currency=usd`, `interval=month`, `interval_count=1`) and **US$60 every year** (`unit_amount=6000`, `currency=usd`, `interval=year`, `interval_count=1`). Annual access saves 50% versus twelve monthly payments. Put their actual Price IDs only in the `STRIPE_PRICE_MONTHLY` and `STRIPE_PRICE_ANNUAL` server secrets. Configure Customer Portal cancellation, payment-method updates, and invoice history. See [PRICING.md](./PRICING.md) for the approved offer and remaining activation gates.
