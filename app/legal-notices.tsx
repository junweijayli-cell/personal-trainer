...
const notices = {
  en: {
    terms: {
      ...
      sections: [
        ...
        ['Trial and proposed subscriptions', `The free trial lasts seven days and begins after successful email verification, using server time. No payment card is required for the trial. The proposed global subscriptions are ${globalPriceLabel('monthly', 'en')} or ${globalPriceLabel('annual', 'en')}. This preview offers clearly labeled Stripe test checkout. Test transactions unlock demo membership and never collect real money. Creating an account or accepting these terms does not charge you. Before paid subscriptions launch, the checkout must clearly show the amount, any applicable tax, renewal conditions, and cancellation terms.`],
    ...
    privacy: {
      ...
      sections: [
        ...
        ['Payment information', 'Stripe hosts demo checkout and billing management. Use test cards only. Stripe receives your email and billing identifiers; TrainWell stores customer, subscription, event and membership records, but never card details. Live payments are disabled.'],
  ...
  zh: {
    terms: {
      ...
      sections: [
        ['关于当前版本', '悦练是一款健身指导应用。本说明适用于当前预览版本。正式运营主体、商业条款以及适用的退款与消费者权利说明，仍需运营方在收费上线前审核。我们不声称此预览版本已满足所有国家的商业运营要求。'],
        ...
        ['试用与拟定订阅', `免费试用为七天，在邮箱验证成功后开始，并以服务器时间计算。试用无需绑定银行卡。全球版拟定价格为 ${globalPriceLabel('monthly', 'zh')} 或 ${globalPriceLabel('annual', 'zh')}。此预览版本提供明确标注的 Stripe 测试支付。测试交易可开启演示会员，不收取真实款项；创建账户或同意本条款不会扣款。正式收费前，支付页面必须清楚展示金额、适用税费、续订条件与取消方式。`],
    ...
    privacy: {
      title: '隐私说明',
      introduction: '以下说明当前悦练预览版如何使用信息来提供账户及训练功能。',
      sections: [
        ...
        ['支付信息', 'Stripe 托管演示支付和账单管理页面，请仅使用测试卡。Stripe 会接收你的邮箱及账单标识；悦练保存客户、订阅、事件和会员记录，但不保存银行卡信息。真实付款未启用。'],
        ['你的控制权', '你可以更新账户信息、拒绝可选健康数据收集、拒绝摄像权限，并使用账户中的导出及删除功能。删除操作会移除应用账户和已保存的记录；服务商备份可能遵循其自身保留计划。正式运营前仍需审核最终数据保留及隐私请求处理流程。'],
        ['联系方式与审核状态', '如需访问、更正或删除数据，或有隐私问题，请发送邮件至 trainwell.win@gmail.com。正式运营主体、适用的国际数据传输披露及最终保留细节尚待运营方审核。本预览说明不代表已经完成监管合规审核。'],
      ],
    },
  },
} as const;
