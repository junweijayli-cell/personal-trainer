  ...
  en: {
    ...
    privacy: {
      ...
      sections: [
        ...
        ['Voice coaching and music', 'Spoken cues use AI-generated Annie (English) and Lea (Chinese) recordings hosted by TrainWell. Playing a cue does not send your profile or workout information to a voice-generation service. Optional voice commands start only when you press Talk to coach and may send audio to your browser’s speech provider for recognition. TrainWell does not save microphone recordings or transcripts. You can use the same coach controls as buttons without a microphone. Background music is generated on your device and can be muted independently.'],
  ...
  zh: {
    terms: {
      ...
      sections: [
        ...
        ['试用与订阅', `七天免费试用在邮箱验证成功后开始，以服务器时间计算，无需绑卡。全球版价格为 ${globalPriceLabel('monthly', 'zh')} 或 ${globalPriceLabel('annual', 'zh')}。购买后付费周期立即开始，试用期间购买也同样如此。月付每月扣款 US$10，年付每年扣款 US$60，直至取消。请在下次续费前通过账户的管理账单取消续订；使用权限保留至已付费周期结束。支付页面在确认付款前展示金额及适用税费。仅创建账户不会扣款。账单或退款问题请联系 trainwell.win@gmail.com，适用的消费者权利不受影响。`],
    ...
    privacy: {
      ...
      sections: [
        ...
        ['语音指导与音乐', '语音提示使用悦练托管的 AI 生成录音：英文 Annie、中文 Lea。播放提示不会向语音生成服务发送你的资料或训练信息。可选语音指令仅在点击“对教练说话”后启动，可能向浏览器语音服务商发送音频进行识别。悦练不保存麦克风录音或转写内容。无需麦克风也可通过按钮使用相同的教练功能。背景音乐在本机生成，可以单独关闭。'],
