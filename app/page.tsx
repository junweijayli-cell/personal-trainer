...
export default function Home() {
  ...
  useEffect(() => {
    ...
    void loadBillingCatalog().then((plans) => {
      if (!cancelled) { setBillingPlans(plans.plans); setBillingMode(plans.mode); setCatalogStatus('ready'); }
    }).catch(() => {
      if (!cancelled) { setBillingPlans([]); setBillingMode(null); setCatalogStatus('unavailable'); }
    });
  ...
  async function removeAccount() {
    if (!member || accountActionBusy) return;
    const confirmed = window.confirm(tr('Delete your TrainWell account and all saved data? Active billing will be canceled. This cannot be undone.', '删除悦练账户及全部数据？当前订阅也会取消，此操作无法撤销。'));
    if (!confirmed) return;
        ...
        <main className="workout-setup">
          ...
          {setupStep === 1 && <section className="setup-panel setup-builder-panel">
            <div className="setup-copy">
              ...
              <p>{tr('Follow TrainWell’s rotation or choose what feels right. Your choice only changes today’s session.', '你可以跟随悦练的轮换推荐，也可以选择今天最想练的部位。')}</p>
          ...
          {setupStep === 2 && <section className="setup-panel setup-choice-panel setup-builder-panel">
            <div className="setup-copy">
              ...
              <p>{tr('Select everything available today. TrainWell will use it where it helps and keep a bodyweight option in every plan.', '选择今天可用的全部器械。悦练会合理使用，并为每个计划保留徒手选项。')}</p>
  ...
  return (
    <main className="coach-app">
      <header className="coach-header">
        <button className="wordmark" type="button" onClick={() => navigate('today')} aria-label={tr('TrainWell home', '悦练首页')}><span>T</span>{tr('TrainWell', '悦练')}</button>
        <p>{trialRemaining !== null ? tr(`${trialRemaining} trial days left`, `试用剩余 ${trialRemaining} 天`) : tr(`${member.membership.plan} member`, `${member.membership.plan === 'daily' ? '日付' : member.membership.plan === 'monthly' ? '月付' : '年付'}会员`)}</p>
      ...
      {view === 'today' && (
        <>
          <section className="today-head">
            <p className="kicker"><i /> {completedToday ? tr('TODAY\'S WORK IS DONE', '今天的训练已完成') : selectedFocus === recommendedFocus ? tr('TrainWell\'s recommendation', '悦练今日推荐') : tr('YOUR CHOICE FOR TODAY', '你今天的选择')}</p>
          ...
          <section className={`account-strip ${accountStatus === 'signed-in' ? 'account-ready' : ''}`}>
            ...
            {(accountStatus === 'signed-out' || accountStatus === 'error') && <>
              ...
              <div><strong>Keep your progress secure</strong><small>{tr('Return to the TrainWell account screen.', '返回悦练账户页面。')}</small></div>
          ...
          <section className="weekly-rotation">
            <div><p className="kicker">{tr('YOUR WEEKLY ROTATION', '你的每周轮换')}</p><h2>{tr('Balanced across the week', '一周均衡训练')}</h2><span>{tr('TrainWell rotates muscle groups so one area is not trained hard every day. Tap any day to use its focus now.', '悦练会轮换训练部位，避免同一部位连续高强度训练。点击任意一天即可使用该计划。')}</span></div>
          ...
          <section className="checkin-section">
            ...
            {accountStatus === 'signed-in' && account ? (
              <form className="checkin-form" onSubmit={saveCheckin}>
                ...
                <label className="health-consent"><input type="checkbox" checked={account.profile.consentHealthData} onChange={(event) => updateProfile({ consentHealthData: event.target.checked })} /><span>{tr('I consent to securely storing this wellness information so TrainWell can personalize my training. I can export or delete it at any time.', '我同意安全存储这些健康信息，以便悦练个性化训练。我可以随时导出或删除。')}</span></label>
      ...
      <footer><span>{tr('TrainWell', '悦练')} / {tr('TRAIN WITH CLARITY', '清晰训练')}</span><span>{tr('PRIVATE BY DESIGN', '隐私优先设计')}</span></footer>
    ...
    <div className={`phase-guide ${compact ? 'phase-guide-compact' : ''}`} data-guide-mode={showVideo ? 'video' : 'photos'}>
      {!compact && exercise.video && !videoFailed && (
        <div className="guide-mode-switch" role="tablist" aria-label={language === 'zh' ? `${exerciseName} 指导方式` : `${exerciseName} guide format`}>
          ...
          <button className={!showVideo ? 'active' : ''} type="button" role="tab" aria-selected={!showVideo} onClick={() => { setGuideMode('photos'); setAutoPlaying(true); }}>
            <span aria-hidden="true">↻</span>{language === 'zh' ? '自动图片示范' : 'Auto photo demo'}
          </button>
        </div>
      )}
      ...
      {!compact && !showVideo && <>
        <div className="phase-cue auto-phase-cue" aria-live="polite">
          <p><span>{language === 'zh' ? phaseChinese[phase.id] : phase.label}</span>{phase.cue}</p>
          <div className="auto-phase-progress" aria-hidden="true">{exercise.phases.map((item, index) => <i className={index === phaseIndex ? 'active' : ''} key={item.id} />)}</div>
        </div>
      </>}
    </div>
  );
  ...
  return (
    <main className="paywall-shell">
      ...
      <section>
        ...
        <p className="account-save-status">{billingMode === 'test' ? tr('Demo checkout · Use Stripe test cards only · No real money is charged', '演示支付 · 仅使用 Stripe 测试卡 · 不会收取真实款项') : tr('Paid membership starts immediately. Renews automatically until canceled in Account → Manage billing.', '付费会员立即开始，自动续费，可在账户的管理账单中取消。')}</p>
    ...
    <div className="preview-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section className="exercise-preview" role="dialog" aria-modal="true" aria-label={`${item.name} movement guide`}>
        ...
        <div className="preview-visual">
          ...
          <span>{item.video ? (language === 'zh' ? '视频 + 自动示范' : 'VIDEO + AUTO DEMO') : (language === 'zh' ? '自动动作示范' : 'AUTO MOVEMENT DEMO')}</span><span>{language === 'zh' ? '先看，再练' : 'LOOK FIRST'}</span>
        </div>
