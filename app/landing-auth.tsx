...
const copy = {
  en: {
    ...
    monthly: 'Monthly', annual: 'Annual', bestValue: 'BEST VALUE', priceTbd: 'Price coming soon', planMonthly: 'Billed monthly in USD. Cancel renewal anytime.', planAnnual: 'Billed yearly in USD. Cancel renewal anytime.', startTrial: 'Start free trial',
  ...
  zh: {
    ...
    step1: '告诉 TrainWell 你有什么', step1c: '选择训练部位、可用器械与时间。',
    ...
    monthly: '月付', annual: '年付', bestValue: '最超值', priceTbd: '价格即将公布', planMonthly: '以美元按月续费，可随时取消续订。', planAnnual: '以美元按年续费，可随时取消续订。', startTrial: '开始免费试用',
    footer: '清晰训练', private: '隐私优先设计',
    createTitle: '创建 TrainWell 账户', createCopy: '邮箱验证后即开始七天免费试用。', name: '姓名', email: '邮箱地址', password: '创建密码', confirm: '确认密码',
    passwordHelp: '至少 10 个字符，并包含大小写字母和数字。', consent: '我同意《使用条款》《隐私政策》及处理我的训练数据。', continue: '发送验证码', haveAccount: '已经有账户？', signInTitle: '欢迎回来', signInCopy: '登录后继续你的训练计划。', noAccount: '第一次使用 TrainWell？', createAccount: '创建账户',
    ...
    forgot: '忘记密码？', forgotTitle: '重置密码', forgotCopy: '输入账户邮箱。如果账户存在，TrainWell 会发送安全重置链接。', sendReset: '发送重置邮件', resetSent: '如果此邮箱存在账户，重置邮件正在发送。', resetTitle: '设置新密码', resetCopy: '为你的 TrainWell 账户创建新密码。', savePassword: '保存新密码', resetDone: '密码已更新，请使用新密码登录。',
...
export default function LandingAuth({ language, onLanguageChange, onAuthenticated, initialMode }: {
  ...
  async function forgotPassword(event: React.FormEvent) {
    ...
    finally { refreshCaptcha(); setNotice(text.resetSent); setBusy(false); }
  }
  ...
  return (
    <main className="landing-shell">
      <header className="landing-nav">
        <button className="wordmark" type="button" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}><span>T</span>TrainWell</button>
