...
export default function LandingAuth({ language, onLanguageChange, onAuthenticated, initialMode }: {
  ...
  return (
    <main className="landing-shell">
      ...
      <footer className="landing-footer"><span>TrainWell / {text.footer}</span><nav className="legal-links" aria-label={language === 'zh' ? '法律与隐私' : 'Legal and privacy'}><button type="button" onClick={() => setLegalDocument('terms')}>{language === 'zh' ? '使用条款' : 'Terms of use'}</button><button type="button" onClick={() => setLegalDocument('privacy')}>{language === 'zh' ? '隐私说明' : 'Privacy notice'}</button></nav><span>{text.private}</span></footer>
