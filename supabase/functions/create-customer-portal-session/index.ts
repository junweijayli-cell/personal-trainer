...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (membership.billing_mode !== mode) throw new Error('Billing mode changed. Refresh your account.');
    const portal = await stripeClient(mode).billingPortal.sessions.create({
