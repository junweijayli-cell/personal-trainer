...
export async function runtime(admin: SupabaseClient) {
  ...
  if (error || data?.mode !== 'test') throw new Error('Test billing is unavailable.');
  return data as { mode: 'test'; checkout_enabled: boolean };
}
...
export async function checkoutAvailable(admin: SupabaseClient, userId?: string) {
  if ((await runtime(admin)).checkout_enabled) return true;
  if (!userId) return false;
  const { data, error } = await admin.from('billing_test_users').select('user_id').eq('user_id', userId).maybeSingle();
  if (error) throw new Error('Demo checkout is unavailable.');
  return Boolean(data);
}
